...
trait HttpTest[F[_]]
  ...
  "parse response" - {
    ...
    "lift errors due to mapping with impure functions into the response monad" in {
      implicit val monadError: MonadError[F] = backend.genericBackend.responseMonad
  ...
  "headers" - {
    ...
    "include a header once when sent effect is used multiple times" in {
      ...
      backend.genericBackend.responseMonad.flatMap(sent)(_ => sent).toFuture().map(_.body).map { body =>
  ...
  if (supportsCancellation) {
    "cancel" - {
      "a request in progress" in {
        implicit val monad: MonadError[F] = backend.genericBackend.responseMonad
        import sttp.monad.syntax._
