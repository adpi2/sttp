...
abstract class BackendStub[F[_], P](
  ...
  class WhenRequest(p: AbstractRequest[_, _] => Boolean) {
    ...
    def thenRespondWithCode(status: StatusCode, msg: String = ""): SelfStubType = thenRespond(
...
class EffectBackendStub[F[_]](
  ...
  override def wrap(f: GenericBackend[F, Any] => GenericBackend[F, Any]): Backend[F] = Backend(f(this))
...
object BackendStub {
  ...
  def synchronous: BackendStub[Identity, Any] = new EffectBackendStub(IdMonad, PartialFunction.empty, None)
  ...
  /** Create a stub backend which delegates send requests to the given fallback backend, if the request doesn't match
    ...
    */
  def withFallback[F[_]](fallback: Backend[F]): EffectBackendStub[F] =
